...
class Vector2$1 {
    ...
    Torwards(vector) {
        ...
        return new Vector2$1(oldX / newX, oldY / newY);
    }

    Away(vector) {
        return this.Torwards(vector).Multiply(new Vector2$1(-1, -1));
    }
}

var Zero = new Vector2$1(0, 0);
var One = new Vector2$1(1, 1);
var Up = new Vector2$1(0, 1);
var Down = new Vector2$1(0, -1);
var Left = new Vector2$1(-1, 0);
var Right = new Vector2$1(1, 0);
...
class Circle extends GameObject {
    ...
    Update() {
        ...
        if (this.position.x + this.radius + canvasMargin >= canvas$1.width) {
            this.position = new Vector2$1(canvas$1.width - 10, this.position.y);
            this.velX *= -1;
        }
        else if (this.position.x - canvasMargin <= 0) {
            this.position = new Vector2$1(10, this.position.y);
            this.velX *= -1;
        }


        if (this.position.y + this.radius + canvasMargin >= canvas$1.height) {
            this.position = new Vector2$1(this.position.x, canvas$1.height - 10);
            this.velY *= -1;
        }
        else if(this.position.y - canvasMargin <= 0) {
            this.position = new Vector2$1(this.position.x, 10);
            this.velY *= -1;
        }

        this.position = new Vector2$1(this.position.x + this.velX, this.position.y + this.velY);
...
class GameControl {
    ...
    InitializeBreakout() {
        ...
        this.CreateBall("circle", null, new Vector2$1(32, 32), new Vector2$1(0, 0), 0, Math.PI * 2, 8, "white");
